 ...
 * The parameters are all passed on the stack and all local variables are stored on the stack. All
 * expressions are evaluated using the stack and two registers rax and rdx Conditionals tests are
 * viewed as expressions returning 0 for false and 1 for true This code can not be used to generate
 * libaries that other C programs can call because it doesn't follow the X86-64 calling convention.
 * The code is printed to standard output as we visit the nodes of the syntax tree.
 * 
 * We maintain a few hashmaps 1. labelMap: to keep track of the labels for class methods this will
 * map method names to their labels 2. varMap: to keep track of the local variables in a method this
 * will map variable names to their negative offsets from the base pointer 3. paramMap: to keep
 * track of the parameters of a method this will map parameter names to their positive offsets from
 * the base pointer
 * 
 * We also need some auxiliary variables to keep track of 1. currClass - the current class 2.
 * currMethod - the current method 3. labelNum - the current label number, for use in loops and
 * conditionals 4. stackOffset - the current stack offset, to keep track of temporaries 5.
 * numFormals - the current number of formal parameters processed 6. formalOffset - the current
 * formal offset, to keep track of parameters
 * 
...
public class CodeGen_Visitor implements Visitor {
    ...
    public Object visit(And node, Object data) {
        ...
        Exp e1 = node.e1;
        Exp e2 = node.e2;
        String exp1 = (String) node.e1.accept(this, data);
        String exp2 = (String) node.e2.accept(this, data);
        return exp1 + exp2 + "# and:" + node.accept(ppVisitor, 0) + "\n" + "popq %rax\n"
                + "popq %rcx\n" + "imulq %rcx, %rax\n" + "pushq  %rax\n";
    ...
    public Object visit(ArrayAssign node, Object data) {
        ...
        String exp1 = (String) node.e1.accept(this, data);
        String exp2 = (String) node.e2.accept(this, data);
        String var = currClass + "_" + currMethod + "_" + i.s;
        String idx = varMap.get(var);
        return exp1 + exp2 + "# " + node.accept(ppVisitor, 0) + "\n" + "popq %rax\n" + "popq %rcx\n"
                + "incq %rcx\n" + "movq " + idx + ", %rdx\n" + "movq %rax,  (%rdx, %rcx, 8)\n";
    }
    ...
    public Object visit(ArrayLength node, Object data) {
        // not in MiniC
        Exp e = node.e;
        String exp = (String) node.e.accept(this, data);
        return exp + "# " + node.accept(ppVisitor, 0) + "\n" + "popq %rax\n" + "movq (%rax),%r8\n"
                + "pushq %r8\n";
    }
    ...
    public Object visit(Assign node, Object data) {
        ...
        String result =
                ...
                expCode + "#" + node.accept(ppVisitor, 0) + "\n" + "popq %rax\n" + "movq %rax, "
                        + location + "\n";
    ...
    public int getNumArgs(ExpList e) {
        /*
         * for the ith argument to the call, store it in regFormals[i] starting with i=0
         */
        if (e == null) {
            return 0;
        } else {
    ...
    public Object visit(Call node, Object data) {
        /*
         * For this implementation, we will assume there are at most 6 arguments to the call. In
         * which case, all arguments get passed into registers. So we will pop the arguments off of
         * the stack and push them into the appropriate registers. ExpList will generate code that
         * evaluates the arguments and pushes them into the stack. If the args are a1,...,ak Then we
         * first pop off a1 and the last to pop off is ak We store them in "rdi", "rsi", ... which
         * are in the regFormals array
         */
        ...
        for (int j = 0; j < numArgs; j++) {
            storeArgsCode += "popq " + regFormals[j] + "\n";
        }

        makeCall = "# calling " + i.s + "\n" + "callq _" + i.s + "\n" // using function name as
                                                                      // label with "_" prefix
                + "pushq %rax\n"; // push the result of the function on the stack

        return evalArgsCode + storeArgsCode + makeCall;
    ...
    public Object visit(ExpList node, Object data) {
        /*
         * ExpList is a left-associative tree, so if node represents a1,a2,...,ak then node.e = ak
         * and node.elist = a1,....,a(k-1) This code will push ak first and a1 last so when we pop,
         * a1 will be the first to be popped and ak the last
         */
        Exp e = node.e;
    ...
    public Object visit(Formal node, Object data) {
        ...
        /*
         * The first six parameters are passed in registers rdi,rsi, rdx,rcx, r7, r8 the rest are
         * passed on the stack above rbp 16(%rbp), 24(%rbp), .. This compiler will copy the
         * parameters in register into the stack frame, so they will be treated as locals!
         */
        Identifier i = node.i;
        Type t = node.t;
    ...
    public Object visit(LessThan node, Object data) {
        /*
         * We implement this in an inefficient but simple way. where we evaluate the two expressions
         * in A<B and push them into the stack as values "a" and "b" then we do a comparison of "a"
         * and "b" and if "a"<"b" we push 1 onto the stack, else we push 0 So the result of LessThan
         * is either 1 (if true) or 0 (if false) This is what C does, it treats 0 as False and
         * nonzero as True
         */
        Exp e1 = node.e1;
        Exp e2 = node.e2;
    ...
    public int getFormalsLocations(FormalList fs) {
        /*
         * getFormals returns the number of formals in the FormalList and uses varMap to assign a
         * stack frame location to each formal variable
         */
        if (fs == null) {
            return 0;
        } else {
    ...
    public String copyFormals(FormalList fs, int n) {
        /*
         * fs.f is the last Formal in the list and fs.flist is the list of earlier formals n is the
         * number of formals in the FormalList so regFormals[n-1] is the register it is stored in
         */
        if (fs == null) {
            return "";
        } else {
            ...
            code += "movq " + register + ", " + location + "  # formal " + formalName + "->"
                    + location + "\n";
            return code;
        }
    }
    ...
    public int getLocalsLocations(VarDeclList vs, int numFormals) {
        /*
         * getLocals returns the number of local variables s in the VarList and uses varMap to
         * assign a stack frame location to each local variable
         */
        if (vs == null) {
            return 0;
        } else {
    ...
    public String initializeLocals(VarDeclList vs) {
        /*
         * look up location of each local variable and store zero in that location
         */
        if (vs == null) {
            return "\n";
        } else {
            ...
            initCode += "movq $0, " + location + "  # " + varName + "->" + location + "\n";
            return initCode;
        }
    }
    ...
    public Object visit(MethodDecl node, Object data) {
        /*
         * The method will have its arguments passed in registers (and we assume for now at most 6
         * arguments) Assume there are r arguments corresponding to the 4 formal parameters of the
         * method a1,..., a4
         * 
         * Our implementation will copy the r arguments to the first 4 slots in the frame: -8(%rbp),
         * -16(%rbp), -24(%rbp) etc. and we will store these locations (whose names a1,...,ar are
         * stored in node.f) using the HashMap varMap. So ai will be in -M(%rbp) where M = 8*i
         * 
         * Next we will initialize the local variables b1,...,bs defined in node.v and store 0 in
         * all of them They will be in the next s slots of the frame The jth local variable will be
         * in -N(%rbp) where N = 8*(r+j)
         * 
         * Next we generate the code for the statements stored in node.s and finally evaluate the
         * expression node.e and return it.
         * 
         * As usual we need a prologue to store the old base pointer %rbp and set it to the new
         * frame, and then to allocate r+s slots in the stack frame for parameters and locals...
         */
        Type t = node.t;
        Identifier i = node.i;
        FormalList f = node.f;
        ...
        String copyFormalsCode = copyFormals(node.f, numFormals);

        String initializeLocalsCode = initializeLocals(node.v);
        ...
        /*
         * VERY SUBTLE POINT!!! before a function call, the stack must be aligned to a 16 byte
         * boundary so we round up the stack change to the nearest multiple of 16 When the call is
         * made the return address is pushed onto the stack then the base pointer is pushed on to
         * the stack So the stack pointer is a multiple of 16 as the return address and the base
         * pointer are 8 byte values. Next we allocate space in the stack frame for the local
         * variables and that must be a multiple of 16 When we prepare for a function call we push
         * the arguments onto the stack. It is usually better to just expand the stack frame by a
         * mutiple of 16 and then move the arguments after the 6th to the correct position relative
         * to the stack pointer, i.e. (%rsp), 8(%rsp), 16(%rsp), 24(%rsp), ... For now, we can
         * assume that we will never have more than 6 arguments
         */
        ...
        prologueCode = "\n\n\n" + ".globl " + theLabel + "\n" + theLabel + ":\n" + "# prologue\n"
                ...
                + copyFormalsCode + "# initialize local variables to zero\n" + initializeLocalsCode;


        epilogueCode = "# epilogue\n" + "popq %rax\n" + "addq $" + (stackChange) + ", %rsp\n"
                + "popq %rbp\n" + "retq\n";

        return prologueCode + statementCode + "# calculate return value\n" + expressionCode
                + epilogueCode;
    }
    ...
    public Object visit(Minus node, Object data) {
        ...
        String minusCode =
                ...
                e1Code + e2Code + "# minus:" + node.accept(ppVisitor, 0) + "\n" + "popq %rdx\n"
                        + "popq %rax\n" + "subq %rdx, %rax\n" + "pushq %rax\n";
    ...
    public Object visit(Not node, Object data) {
        ...
        Exp e = node.e;
        String exp = (String) node.e.accept(this, data);
        return exp + "# not:" + node.accept(ppVisitor, 0) + "\n" + "popq %rax\n" + "negq %rax\n"
                + "addq $1, %rax\n" + "pushq %rax\n";
    }
    ...
    public Object visit(Plus node, Object data) {
        ...
        String addCode =
                ...
                e1Code + e2Code + "# plus:" + node.accept(ppVisitor, 0) + "\n" + "popq %rdx\n"
                        + "popq %rax\n" + "addq %rdx, %rax\n" + "pushq %rax\n";
    ...
    public Object visit(Print node, Object data) {
        ...
        String result =
                ...
                // expCode + "# " + node.accept(ppVisitor, 0) + "popq %rdi\n" + "movb $0, %al\n"
                // + "callq _print\n";
    ...
    public Object visit(Times node, Object data) {
        ...
        String timesCode =
                ...
                e1Code + e2Code + "# times:" + node.accept(ppVisitor, 0) + "\n" + "popq %rdx\n"
                        + "popq %rax\n" + "imulq %rdx, %rax\n" + "pushq %rax\n";
    ...
    public Object visit(While node, Object data) {
        ...
        Statement s = node.s;
        String exp = (String) node.e.accept(this, data);
        String statement = (String) node.s.accept(this, data);
        String label1 = "L" + labelNum;
        labelNum += 1;
        String label2 = "L" + labelNum;
        labelNum += 1;
        return "# while: " + node.accept(ppVisitor, 0) + "\n" + label1 + ":\n" + exp + "popq %rax\n"
                + "cmpq	$1, %rax\n" + "jne " + label2 + "\n" + statement + "jmp " + label1 + "\n"
                + label2 + ":\n";
    }
